...
## 4. Arquitectura de estado en el frontend (React + Zustand + Immer) — IMPLEMENTADO en Fase B, revisado para la vista de hoja doble (spread)

- **Store por página, no store global de la publicación.** `frontend/src/store/pageEditorStore.js` exporta `createPageEditorStore()` -- una FÁBRICA (`create(immer((set, get) => ({...})))` envuelta en una función), no un store singleton. Cada instancia tiene scope a UN `pageId` a la vez: `loadPage()` reemplaza TODO el estado (nunca lo mezcla con el anterior), `loadToken` descarta respuestas de red obsoletas si el usuario ya navegó a otra página antes de que la petición anterior resolviera (ver la "regla de oro" del bug histórico portada↔contraportada, sección 1). Se conserva `usePageEditorStore = createPageEditorStore()` como alias de compatibilidad, sin uso dentro del repo.
- **Por qué fábrica y no singleton (añadido en la ronda de la vista de hoja doble)**: Carlos pidió que, en páginas interiores, el editor muestre dos páginas lado a lado (spread), AMBAS editables independiente y SIMULTÁNEAMENTE (decisión explícita: "como InDesign", no solo una "activa" con clic para enfocar). Un store global no puede sostener dos páginas cargadas a la vez sin repetir el bug de fuga que motivó la regla de oro. `CanvasEditorV2.jsx` crea dos instancias estables vía `useState(() => createPageEditorStore())` (`useLeftStore`/`useRightStore`, nunca en un array o hook condicional) -- en hoja simple (portada, contraportada, o el último spread interior impar con un solo lado) solo la izquierda está en uso; en spread, cada una carga/guarda/hace heartbeat de su propia página exactamente igual que el store único lo hacía antes para una sola página. El bloque de render de una página (Stage/Layer/Transformer/marquee-select de Konva) se extrajo a `<PageCanvas useStoreHook={...}>` para montarlo una o dos veces sin duplicar código; cada instancia tiene sus propios `stageRef`/`trRef`/`shapeRefs`. El rail de herramientas y el panel de propiedades actúan sobre la página "enfocada" (`focusedSide`, estado local del componente que cambia al hacer clic en cualquiera de los dos Stages). El lock de edición (`lockAPI`) sigue siendo por PUBLICACIÓN, no por página -- no necesitó cambios, ya cubría ambas páginas del spread.
...
- **Undo/redo** como pila de snapshots inmutables (Immer `patches`) -- aún no implementado.
- **Shell de UI y navegación entre páginas (revisado en la ronda de la vista de hoja doble)**: la barra lateral con la lista completa de páginas (`.editor-v2-sidebar`/`.editor-v2-pagelist`) se retiró por pedido explícito de Carlos, para ganar espacio de canvas. La sustituyen (a) un encabezado compacto de una sola línea (`.editor-v2-header`, título de la publicación + "Volver a publicaciones") y (b) una barra de navegación inferior (`.editor-v2-pagenav`: "← Anterior", campo numérico editable que salta directamente a la hoja que contiene esa página posicionándola en el lado correcto del spread, "Siguiente →"). La agrupación de páginas en "vistas" (hoja simple vs. spread) la calcula `computeSpreadViews(pages, total)` -- función pura sin dependencias de React, exportada desde `CanvasEditorV2.jsx`: `page_number === 1` (portada) y `page_number === total` (contraportada) siempre solas; el resto se agrupa de a 2 en orden (2,3), (4,5)... con el último spread interior impar quedando con un solo lado.
